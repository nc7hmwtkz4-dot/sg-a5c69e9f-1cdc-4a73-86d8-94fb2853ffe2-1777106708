...
Première passe terminée sur `src/pages/simulateur.tsx` et `src/services/learningEngineService.ts`. Les traces restantes ont ensuite été nettoyées dans `src/pages/upload.tsx` et `src/pages/api/run-learning.ts`, puis validées avec `check_for_errors`.

## Checklist
- [x] Retirer les logs de debug verbeux dans le simulateur et les services métier critiques
- [x] Supprimer les variables, constantes et branches devenues inutiles après les correctifs récents
- [x] Rechercher les traces restantes de code mort ou obsolète dans l’application
- [x] Vérifier que le nettoyage ne modifie pas le comportement utilisateur
- [x] Valider avec check_for_errors et marquer la tâche terminée
